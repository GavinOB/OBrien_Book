\documentclass[11pt,oneside]{memoir}
%\setstocksize{9.25in}{6.125in}
%\settrimmedsize{9in}{6in}{*}
\usepackage[USenglish]{babel}
\usepackage{csquotes}
\usepackage[en-US]{datetime2}
\usepackage[T1]{fontenc}
\usepackage[textlf]{MinionPro}
\usepackage{pdfpages}
\usepackage{dpfloat}
\usepackage[super,comma,square]{natbib} % superscript numerical citations
\renewcommand*{\bibfont}{\raggedright\small}
\newcommand{\bb}[1]{\textbf{\hyperpage{#1}}} % needed for bold & hyperlinked index pages
\usepackage{graphicx}
\graphicspath{ {./images/} }
\usepackage[final]{microtype}
\usepackage{imakeidx}
\makeindex[intoc]
% \showindexmarks 
\frenchspacing

%\usepackage{draftwatermark}
%\SetWatermarkText{Draft}
%\SetWatermarkScale{2}
%\SetWatermarkColor[gray]{0.9}

\usepackage[pagebackref]{hyperref}
\hypersetup{
	colorlinks = true,
	urlcolor = blue,
	linkcolor = blue,
	citecolor = blue
}
\urlstyle{same}

% Don't reset person numbering between chapters
\counterwithout{section}{chapter}

% Set custom page margins
%\setlrmarginsandblock{0.75in}{0.5in}{*}
%\setulmarginsandblock{0.5in}{0.5in}{*}
%\checkandfixthelayout

% Smaller text for quote & quotation blocks
\let\quoteOLD\quote
\def\quote{\quoteOLD\small}
\let\quotationOLD\quotation
\def\quotation{\quotationOLD\small}

% Rename "Bibliography"
\addto{\captionsUSenglish}{%
	\renewcommand{\bibname}{Citations}
}

% Font styling for people and numbered lists
\newcommand{\MainPerson}[1]{\textbf{\textsc{#1}}}
\newcommand{\Lineage}[2]{\textit{#2}{\textsuperscript{#1}}}
\newcommand{\KidNum}[2]{\noindent\makebox[0.65in][l]{\makebox[0.55in][r]{#1\hfill #2}}}
\newcommand{\KidName}[1]{\textsc{#1}}
\newcommand{\GrandkidName}[1]{\textit{#1}}
%\newcommand{\GrandkidNum}[1]{\noindent\makebox[0.9in]{\makebox[0.75in][r]{\hfill #1}}}
\newcommand{\GrandkidNum}[1]{\noindent\makebox[1.1in]{\makebox[0.95in][r]{\hfill #1}}}

% Font styling for kids and grandkids sections
\newenvironment{KidsIntro}
	{
	\vspace{0.5\baselineskip}
	\begin{small}
	}
	{
	\par
	\end{small}
	\vspace{0.5\baselineskip}
	}
\newenvironment{Kids}
	{
		\begin{small}
		\begin{hangparas}{0.65in}{1}
	}
	{
		\end{hangparas}
		\end{small}	
	}
\newenvironment{KidsMoreText}
	{
		\setlength{\parindent}{1.0in}
	}
	{
	
	}

\newenvironment{GrandkidsIntro}
{
	\vspace{0.5\baselineskip}
	\setlength{\parindent}{0.65in}
	}
	{
		\par
	\vspace{0.5\baselineskip}
}

\newenvironment{Grandkids}
	{
		\begin{small}
		\begin{hangparas}{1.1in}{1}
	}
	{
		\end{hangparas}
		\end{small}
		\vspace{0.5\baselineskip}
	}

 \begin{document}

\frontmatter

\author{Gavin E. O'Brien}
\title{Descendants of William O'Brien of Watergrasshill, Ireland and Boston, Massachusetts}
\date{\today}

\maketitle
\tableofcontents

\renewcommand\bibsection{\section*{Citations}} % remove citations from table of contents

\include{./Intro/Intro}

\cleardoublepage
\addcontentsline{toc}{chapter}{Chart: First Three Generations}
\includepdf[pages=1-2]{descendant.pdf}

\listoffigures

\mainmatter
% \include{./Chapter01/chapter01_test}
\include{./Chapter01/O'Briens_in_Ireland}
\include{./Chapter02/O'Briens_in_Boston}
\include{./Chapter01/Name_and_Lineage}
\chapter{First Generation}
\include{./Chapter03/William1OBrien}
\chapter{Second Generation}
\include{./Chapter04/Abigail2OBrien}
\include{./Chapter04/John2OBrien}
\include{./Chapter04/Edward2OBrien}
\include{./Chapter04/Mary2OBrien}
\include{./Chapter04/Michael2OBrien}
\chapter{Third Generation}
\include{./Chapter05/Margaret3Dooley}
\include{./Chapter05/William3OBrien}
\include{./Chapter05/John3OBrien}
\include{./Chapter05/Hannah3Ward}
\include{./Chapter05/Michael3OBrien}
\include{./Chapter05/Frances3OBrien}
\include{./Chapter05/Edward3OBrien}
\chapter{Fourth Generation}
\include{./Chapter06/Catharine4Simonds}
\include{./Chapter06/Abigail4Simonds}
\include{./Chapter06/Caroline4Fernald}
\include{./Chapter06/Sarah4Fernald}
\include{./Chapter06/Robert4Fernald}
\include{./Chapter06/Francis4OBrien}
\include{./Chapter06/Pauline4OBrien}
\include{./Chapter06/Mildred4OBrien}
\include{./Chapter06/MaryAlice4Flynn}
\include{./Chapter06/Edith4Wickens}
\include{./Chapter06/Albert4Wickens}
\include{./Chapter06/Ethel4Wickens}
\include{./Chapter06/Catherine4OBrien}
\include{./Chapter06/Frances4OBrien}
\include{./Chapter06/Edward4OBrien}
\include{./Chapter06/Beatrice4OBrien}
\appendix
\include{./Appendix/cemetery}
\include{./person_list}

\backmatter

\include{./bibliography}
\include{./credits}

\cleardoublepage

\indexprologue{Note: \textbf{bold} page numbers indicate the individual's main profile. Superscripted numbers show the individual's generation, starting with 1 for the oldest generation (William\textsuperscript{1} O'Brien).}
\printindex

\end{document}

